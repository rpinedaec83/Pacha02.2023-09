...
router.get('/', async(req,res)=>{
    const query= await querys.getAllBckubigeo();
    return res.status(200).json(query);
});

router.get('/:ubigeo',async(req,res)=>{
    const {ubigeo} = req.params;
    const query=await querys.getBckubigeoId(ubigeo);
    if(query.length === 0){
...
const query=await querys.createBckubigeo(ubigeo,ubicacion,activo,usuariocreacion);
return res.status(202).json(query);
});

router.put('/',async(req,res)=>{
    const {ubigeo,ubicacion,activo,usuariocreacion}= req.body;
    const query =await querys.updateBckubigeo(ubigeo,ubicacion,activo,usuariocreacion);
    if (query===null){

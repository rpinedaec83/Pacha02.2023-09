...
router.get('/',async(req,res)=>{
    const query=await querys.getAllSexo();
    return res.status(200).json(query);
})
router.get('/:id',async(req,res)=>{
    const {id} = req.params;
    const query=await querys.getSexoId(id);
    if(query.length === 0){
...
const query=await querys.createSexo(descripcion,activo,usuariocreacion);
return res.status(202).json(query);
});
router.put('/',async(req,res)=>{
    const {id,descripcion,activo,usuariocreacion}= req.body;
    const query =await querys.updateSexo(id,descripcion,activo,usuariocreacion);
    if (query===null){

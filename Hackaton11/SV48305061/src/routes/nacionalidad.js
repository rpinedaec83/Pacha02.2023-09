...
router.get('/',async (req,res)=>{
    const query=  await querys.getAllNacionalidad();
    return res.status(200).json(query);
})
router.get('/:id',async(req,res)=>{
    const {id} = req.params;
    const query=await querys.getNacionalidadId(id);
    if(query.length === 0){
...
const query=await querys.createNacionalidad(descripcion,activo,usuariocreacion);
return res.status(202).json(query);
});
router.put('/',async(req,res)=>{
    const {id,descripcion,activo,usuariocreacion}= req.body;
    const query =await querys.updateNacionalidad(id,descripcion,activo,usuariocreacion);
    if (query===null){
